...
_資料區間：2026-04-24 ~ 2026-05-24（30 天）· 樣本：1000 個 repo_

---

## 摘要

本研究蒐集 GitHub 最近 30 天新建立、Stars 大於 10、非 fork 的公開 repository 共 **1000 筆**，分析其程式語言、主題標籤、熱度指標與類別分布。
...
3. **類別**：經規則式分類（2026-05-25 修訂為 10 類，新增 Finance/Trading），**AI/ML (38.9%) 與 Other (37.5%) 合占 76.4%**；本輪新獨立出 **Finance/Trading 類 (4.5%)** 主要為加密交易/套利機器人聚落。
4. **相關性**：Stars↔Forks 為中度正相關（Pearson 0.35 / Spearman 0.43），非強相關；類別內部呈現極端兩極化 — AI/ML 類 fork:star 僅 **18%**（「被星但少被用」），Finance/Trading 類則為 **767%**（median 9.9×，疑似自動化 fork-farming，見 §9.3）。
5. **補章原創貢獻**：以 8 訊號加權的**公開 metadata 完整度風險評分**量化「stars 相對於可見 metadata 訊號偏稀疏」的程度，1.8% (18 repos) 落入「低資訊密度」候選 tier，集中於 1000-4999 stars 級距 (9.9%)。Finance/Trading 類經單獨檢視為 metadata 完整度最佳之一（mean score 0.78 vs 整體 1.35），與「fork-farming 疑似」共構出特殊型態：「metadata 漂亮 + fork 暴衝 + stars 平庸」。
...
- AI/ML 類別有一明顯子群「stars 高但 forks 偏低」（左上方稀疏區，fork:star 18%）
- **Finance/Trading 類則呈現完全相反的反向異常 — 高 forks 低 stars（fork:star 767%，median 9.9×）**，疑似自動化 fork-farming，詳見 §9.3
- 結論：**Star ≠ 真實採用度**，**Forks ≠ 真實採用度**，這也是補章 metadata 完整度分析的理論基礎。

### 7.5 類別分布與熱度（RQ5、RQ6）

![Category Distribution](figures/category_distribution.png)

_資料快照：2026-05-25；ADR-0004 修訂版分類器（10 類，新增 Finance/Trading）。_

| 分類 | 數量 | 占比 | 平均 Stars | 中位數 Stars | 平均 Forks | 平均 Stars/day | fork:star |
|---|---:|---:|---:|---:|---:|---:|---:|
| **AI/ML** | **389** | **38.9%** | **602** | **231** | **106** | **41.0** | **18%** |
| Other | 375 | 37.5% | 390 | 224 | 109 | 40.7 | 28% |
| Web | 50 | 5.0% | 397 | 217 | 107 | 28.3 | 27% |
| **Finance/Trading** | **45** | **4.5%** | 223 | 189 | **1711** | 53.4 | **767% 🚩** |
| Mobile | 42 | 4.2% | 402 | 217 | 23 | 26.8 | 6% |
| CLI/Tooling | 30 | 3.0% | 438 | 238 | 120 | 41.6 | 27% |
| Game | 25 | 2.5% | 788 | 268 | 55 | 89.1 | 7% |
| Security | 22 | 2.2% | 453 | 247 | 129 | 39.2 | 28% |
| Data | 11 | 1.1% | 678 | 327 | 85 | 62.4 | 13% |
| DevOps | 11 | 1.1% | 324 | 213 | 89 | 16.5 | 27% |

![Category Avg Stars](figures/category_avg_stars.png)

**回答 RQ5（類別熱度差異）**：
- **Game** 平均 stars 最高 (788)，但被 `antirez/ds4` (11711 stars) 異常值拉抬，看中位數 (268) 較中肯
- **Data** 樣本最少 (11) 但中位數最高 (327)，少而精
- **AI/ML** 平均 stars 高 (602) 但平均 forks 僅 106，fork:star 比例 18% 屬正常類別中最低
- **Finance/Trading** 平均 forks 1711 是所有類別中最高的，但 stars 中位數僅 189 — **這個反向比例與其他類別完全不符**，詳見 §9.3

**回答 RQ6（趨勢偏向）**：
**AI/ML (38.9%) + Other (37.5%) 合計 76.4%**，2026 trending 主結構為：
1. AI 應用爆發（agent、LLM、claude-code 生態）— AI/ML 為最大類別
2. 大量未標籤、難分類的長尾 repo — Other 緊追在後
3. 本輪修訂後新獨立出的 **Finance/Trading 類 (4.5%)** — 主要為 polymarket / 加密套利機器人聚落，從原本被埋沒於 Other 桶釋出

其餘 6 個傳統類別合計不到 18%。**Web 已不再是 trending 主軸**（僅占 5.0%）。
...
### 9.1 主結構：AI 主導 + 加密交易暗流 + 長尾

76.4% 的 trending repo 落在 AI/ML 與 Other 兩個桶；本輪 ADR-0004 修訂後從 Other 釋出的 Finance/Trading 類占 4.5%；其他 7 個傳統類別加起來不到 18%。這不是分類器太粗糙的問題（已對 description + topics + language 三個來源做關鍵字匹配，並於 2026-05-25 補齊現代 stack 缺漏），而是**生態本身的真實偏移** —— 2026 上半年的開源動能集中在 AI 應用，且大量新 repo 因為快速出貨缺乏標準的 metadata。Finance/Trading 類獨立後浮現出一個本來看不到的子聚落（45 筆 polymarket / 加密套利機器人），詳見 §9.3。

### 9.2 fork:star 的兩極化現象

| 類別 | fork:star | 直觀解讀 |
|---|---:|---|
| AI/ML | **18%** | 高 stars 低 forks — 「被星但少被用」 |
| Mobile | 6% | 低 forks 反映 app store 部署不需 fork |
| 其他正常類別 | 5-30% | 一般 baseline 區間 |
| **Finance/Trading** | **767%** 🚩 | 異常反向 — 詳見 §9.3 |

AI/ML 類別 fork:star 18%，可能解釋：
...
### 9.3 Finance/Trading 類的反向 fork 異常：疑似 fork-farming

本輪 ADR-0004 修訂後新獨立出 Finance/Trading 類（45 筆），檢視其 fork 模式發現**與所有其他類別完全相反的型態**：

- **平均 forks 1711、stars 僅 223** — fork 是 stars 的 7.67×
- **Median fork:star = 9.9×；p25 = 4.07×, p75 = 10.79×** — 不是少數 outlier，整類都這樣
- **Top 5 polymarket bot：fork 全部超過 3000，stars 都只有 100-300 區間**
  - `POLYMARKET-TRADER-LAB/Polymarket-trading-bot`: 289 stars / 4262 forks（14.7×）
  - `mclaeo/polymarket-copytrade`: 144 / 3918（27.2×）
  - `mclaeo/polymarket-arbitrage`: 146 / 3892（26.7×）
  - `md0070/polymarket-trading-bot`: 239 / 3098（13.0×）
  - `OpenPatchLab/polymarket-weather-trading-bot`: 147 / 2843（19.3×）

正常 repo 的 fork:star 不會超過 100%（fork 一個 repo 比 star 它麻煩太多），這個 cluster 的 1000-2000% 區間幾乎可確定**不是有機行為**。三種可能解釋：

1. **自動化 fork-farming**：以大量機器人帳號 fork 提升曝光、塞滿「trending」演算法的 fork 訊號（GitHub trending 排序部分權重來自 fork 速度）。
2. **「下載即 fork」的部署 pattern**：使用者要部署自己版本必須 fork 然後改設定，是真實採用，但完全不 star（因為早期版本還沒成熟到值得 star）。但這無法解釋為何 fork 數會數倍於 stars 還 collapse 在 10-30× 這麼集中的 ratio。
3. **GitHub 演算法 / 抓取異常**：可能性最低，因為 Search API 直接抓 `forks_count`，多個 repo 同 pattern 不像 API bug。

**本研究的立場**：無法區分 (1) 與 (2)，但建議讀者對該類數據保留警戒、**不要把 forks 數當作真實採用量**。後續可比對 fork 帳號的建立時間、commit history、followers 數深入驗證 — 屬於 future work。

附帶觀察：Finance/Trading 類的 **metadata 完整度評分反而是所有類別中最好的之一**（mean score 0.78 vs 整體 1.35，全部 45 個都在「訊號完整」tier）— 這與「fork-farming 疑似」共構出一個特殊型態：「metadata 漂亮 + fork 暴衝 + stars 平庸」，可能反映「**精心打理門面以吸引部署**」的策略。

### 9.4 Metadata 完整度補章對主分析的意義
...
54.4% repo 完全沒 topic。這不是抓取錯誤，是 GitHub 生態的現實：topic 是選填欄位，新建 repo 通常不會立刻填。任何完全依賴 topic 的分類器都會嚴重低估覆蓋率。本研究的規則式分類器同時使用 description + topics + language 三個來源，部分緩解了此問題，但 37.5% 的 Other 比例仍反映了這個侷限（2026-05-25 ADR-0004 修訂從 42.0% 降至 37.5%）。
...
| D2 | 54.4% repo 無 topics | Topic 為選填欄位，是 GitHub 生態結構性現象；限制 topic-based 分析廣度，且 Other 桶比例偏高（修訂後 37.5%）部分源於此 |
...
| M2 | 單一分類 MVP | React + LLM 的 repo 會優先歸 AI/ML（優先順序高），**可能系統性高估 AI/ML 類別占比、低估被「吞掉」的 Web/Mobile/Data 等類別**。2026-05-25 修訂後新增 Finance/Trading 類，類別總數從 9→10；priority 為 AI/ML > Security > Finance/Trading > DevOps > Data > Web > Mobile > Game > CLI/Tooling > Other |
| M3 | 分類結果未經人工抽樣驗證 | 目前無法提供 precision/recall 數字；隨機 50 筆驗證樣本已 scaffold 於 `data/processed/classification_validation_sample.csv`（10 類 × 5 筆 stratified），待人工標註後回填一致率 |
...
本研究成功蒐集 1000 個近月熱門 GitHub repo 並完成系統性分析。主要結論：
...
3. **熱度相關性**：Stars↔Forks 為中度正相關（0.35），但類別內部呈現極端兩極化 — AI/ML「被星但少被 fork」（18%），Finance/Trading 反向異常（767%，疑似 fork-farming）。
4. **類別趨勢**：AI/ML + Other 占 76.4%；本輪 ADR-0004 修訂後新獨立出 Finance/Trading 類 (4.5%)，呈現「AI 主導 + 加密交易暗流 + 長尾」結構。**2026 上半年 GitHub trending 已被 AI 主導，但 polymarket / 套利機器人子聚落為過去未被注意到的訊號**。
5. **原創貢獻 A**：以 8 訊號加權的公開 metadata 完整度風險評分量化 trending repo 的 metadata 完整度，1.8% 落入「低資訊密度」候選 tier，集中於 1000-4999 stars 級距 (9.9%)。
6. **原創貢獻 B（修訂版新增）**：浮現 Finance/Trading 類異常 fork:star 比例的觀察（median 9.9×，top 5 polymarket bot 達 13-27×），疑似自動化 fork-farming；建議讀者不要把 forks 數當作真實採用量，並開啟 future work 比對 fork 帳號特徵驗證。

研究成果包含：
- 1000 筆 repo 資料集（CSV + 原始 JSON）
- 7 張視覺化圖表
